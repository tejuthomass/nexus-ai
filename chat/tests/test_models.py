...
class DocumentModelTest(TestCase):
    ...
    @patch('chat.signals.cloudinary')
    def test_document_cascade_delete_on_session_delete(self, mock_cloudinary, mock_delete_vectors):
...
class ModelRelationshipTest(TestCase):
    ...
    def test_session_has_multiple_documents(self, mock_cloudinary):
        ...
        for i in range(3):
            test_file = SimpleUploadedFile(
                f'doc{i}.pdf', b'content', 'application/pdf'
            )
            Document.objects.create(
                session=session, file=test_file, title=f'Doc {i}'
            )
    ...
    @patch('chat.signals.delete_session_vectors')
    @patch('chat.signals.cloudinary')
    def test_cascade_delete_user_removes_all_related(self, mock_cloudinary, mock_delete_vectors):
